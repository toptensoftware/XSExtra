	...
	public class AddinCommandHandler : NodeCommandHandler
	{
		...
		protected void OnSyncWithFileSystem()
		{
			...
			using (var monitor = IdeApp.Workbench.ProgressMonitors.GetToolOutputProgressMonitor(true))
			{
				...
				using (var output = monitor.Log)
				{


					output.WriteLine("Synchronizing folder {0}...", folder.Path.ToString());
					...
					output.WriteLine(string.Format("Finished - {0}", message));
				}
			}
		}
	}
